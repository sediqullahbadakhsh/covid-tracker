import actionTypes from "./covideActionTypes";
import covidDataReducer from "../../redux/covid/covid";
...
describe("Redux should work properly", () => {
  it("it should dispatch fetch start action properly ", () => {
    covidDataReducer(state, { type: actionTypes.FETCH_COVID_DATA_START });
    expect(state.loading).toBeTruthy();
  });
  it("should dispatch add covid data properly", () => {
    const store = covidDataReducer(state, {
      type: actionTypes.ADD_COVID_DATA,
  ...
  it("should dispatch an error while fetching", () => {
    const store = covidDataReducer(state, {
      type: actionTypes.COVID_DATA_ERROR,
